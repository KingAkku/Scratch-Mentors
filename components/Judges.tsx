...
const JudgeCard = ({ judge }: { judge: typeof JUDGES[0] }) => {
  return (
    <div className="group relative w-full max-w-sm mx-auto h-full">
      ...
      <div className="absolute -inset-0.5 bg-gradient-to-r from-purple-400 to-blue-400 rounded-xl opacity-0 group-hover:opacity-30 blur transition duration-500"></div>
      ...
      <div className="relative h-full bg-white border border-gray-100 shadow-md rounded-xl overflow-hidden flex flex-col p-6 transition-transform duration-300 group-hover:-translate-y-2">
        ...
        {/* Avatar Section */}
        <div className="relative w-32 h-32 mx-auto mb-6 rounded-full p-1 border-2 border-dashed border-gray-300 group-hover:border-purple-500 transition-colors duration-300">
           <img 
             src={judge.image} 
             alt={judge.name}
             className="w-full h-full rounded-full object-cover grayscale group-hover:grayscale-0 transition-all duration-500 bg-gray-100"
           />
           {/* Rotating ring element */}
           <div className="absolute inset-0 border-r-2 border-transparent border-t-2 border-purple-500 rounded-full animate-spin-slow opacity-0 group-hover:opacity-100 transition-opacity"></div>
        </div>

        {/* Info Section */}
        <div className="text-center mb-6">
          <h3 className="text-xl font-bold text-black mb-2 group-hover:text-purple-700 transition-colors leading-tight">{judge.name}</h3>
          <p className="text-purple-600 font-mono text-xs uppercase tracking-wider mb-1 font-bold">{judge.role}</p>
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-6">
           {judge.tags.map(tag => (
             <span key={tag} className="px-2 py-1 bg-gray-100 border border-gray-200 rounded text-[10px] text-gray-600 font-mono uppercase">
               {tag}
             </span>
           ))}
        </div>
        ...
        <div className="mt-auto pt-4 border-t border-gray-100 flex justify-center text-gray-400">
           <a 
             href={judge.linkedin} 
             target="_blank" 
             rel="noopener noreferrer" 
             className="group/link flex items-center gap-2 px-4 py-2 hover:bg-gray-50 rounded-full transition-all hover:text-purple-600"
           >
             <Linkedin size={16} />
...
export const Judges: React.FC = () => {
  ...
  return (
    <section ref={sectionRef} className="py-24 bg-white relative border-t border-gray-100">
      {/* Decorative Background Elements */}
      <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')] opacity-[0.03] pointer-events-none"></div>
      <div className="absolute left-0 top-1/2 -translate-y-1/2 w-64 h-64 bg-purple-100 rounded-full blur-[100px] pointer-events-none"></div>
